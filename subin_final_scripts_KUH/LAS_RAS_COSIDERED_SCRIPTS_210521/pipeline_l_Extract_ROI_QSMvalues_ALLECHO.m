...
min_roi_qsmvalues = zeros(length(subjects), length(regionnames));
max_roi_qsmvalues = zeros(length(subjects), length(regionnames));
sd_roi_qsmvalues  = zeros(length(subjects), length(regionnames));
 
for idx = 141:length(subjects) %1:length(subjects)
...
%         %---Do for orig QSM ROI extraction----(do only once)
%         % Make into RAS
%         qsm_las2ras = qsm;
%             qsm_las2ras.img = flipud(qsm.img); %LAS-->RAS
%             fname_orig_qsm_RAS = [dirs.qsmnet '\' subj '_QSMnet+_64_25_bet_qs_ALLECHO' '_RAS.nii'];
%             save_nii(qsm_las2ras, fname_orig_qsm_RAS);
%         % Load; Check if orientation is ok.
%         qsm = load_nii(fname_orig_qsm_RAS);
%             figure; imshow3d(qsm.img(:,:,1:5:end));
%             figure; imshow3d(single(fsroi.img(:,:,1:5:end)));
%         %-----------------------------
...
%         fprintf ('--doing region: %s ---\n', regionnames{ii})
        ...
        roimask = fsroi.img;
            ...
            roimask(find(ismember(roimask,ROI_label))) = 1;
                ...
                if isempty(roi_qsmvalues) == 1
                    fprintf('skipping %s because empty', ROI)
                else
                    average_roi_qsmvalues(idx,ii) = mean(roi_qsmvalues);
                    min_roi_qsmvalues(idx,ii) = min(roi_qsmvalues);
                    max_roi_qsmvalues(idx,ii) = max(roi_qsmvalues);
                    sd_roi_qsmvalues(idx,ii) =  std(roi_qsmvalues);
                end
