...
dirs.DDRIVE = 'D:\subindata\KUH\';
    dirs.orig_qsm_dcm  = [dirs.NAS '0_dcm\Prospective']; %['Z:\Personal_Folder\Eun-Jung\Complete\[KUH]QSMnet_plus_Siemens\201911\SIEMENS_QSM_Rawdata']; %[dirs.NAS '0_dcm'];
    ...
    dirs.mag_echo_1iso       = [dirs.DDRIVE '2_New_210331\' '2_qsm_mag_2nd_echo_1iso'];
    dirs.mag_coreg_fsmask    = [dirs.DDRIVE '2_New_210331\' '3_2_fsmasks_fsspace_magcoreg'];
    dirs.vsharp_fsbrainmask  = [dirs.DDRIVE '2_New_210331\' '3_2_fsmasks_fsspace_vsharp'];
    dirs.phs_tissue          = [dirs.DDRIVE '2_New_210331\' '4_QSMnet_input_phstissue'];    
    dirs.steps               = [dirs.phs_tissue '\steps'];
    dirs.qsmnet    =  [dirs.DDRIVE '2_New_210331\' '5_0_QSMnet_outputs'];
    dirs.qsmnet_ro =  [dirs.DDRIVE '2_New_210331\' '5_1_uzip_ro_ud_QSMnet_files'];
    dirs.zmask           = [dirs.DDRIVE '2_New_210331\' '2_qsm_zeromask'];
...
suffix.qsmnetout_ro_ud_zp = ['_QSMnet+_64_25_fswmparc' '_dilated' options.dilatenum_str '_ro_ud_zp'];
if options.do_erode ==1
    ...
    suffix.qsmnetout_ro_ud_zp = ['_QSMnet+_64_25_fswmparc' '_dilated' options.dilatenum_str '_eroded' options.erodenum_str '_ro_ud_zp'];
end
...
for idx = 1:length(subjects)
 % 0. Define Filenames:
    subj = subjects{idx};
        ...
        fname_coreg_wmparc       = [dirs.mag_coreg_fsmask '\magcoreg_nn_' subj suffix.FSroimask '.nii'];
        ...
        fname_qsmnet_ro_ud_zp = [dirs.qsmnet_ro '\' subj suffix.qsmnetout_ro_ud_zp '.nii']; 
        
        fname_zeromask   = [dirs.zmask '\zeromask_' subj suffix.mag_echo_1iso '.nii'];
        fname_v_z_fsmask = [dirs.v_z_fsbrainmask '\v_z_magcoreg_nn_' subj suffix.FSroimask '.nii'];
        
        orig_qsm_dcm_dir_subj = [dirs.orig_qsm_dcm '\' subj '\QSM']; 
        %orig_qsm_dcm_dir_subj = [dirs.orig_qsm_dcm '\' subj(1:3) '_' subj(4:end) '\DICOM']; %'\QSM']; 
        
% 1. Preprocess QSM        
%     pipeline_h_QSM_preprocessing_KUH

% 2. Resize phs_tissue for QSMnet+
%     pipeline_i_Resize_phstissue_for_QSMnet 
    
% 3. Run spyder ('inference_subin_FS_kuh_final.py'), then run below

% 4. Postprocess QSMnet_ output
   pipeline_j_Coregister_QSMnet_to_T1_final

% 5. Postprocess ROImask to completely overlap with QSMnet_output
    pipeline_k_make_zeromask
...
% 6. Extract QSM values from ROIs:
    pipeline_l_Extract_ROI_QSMvalues