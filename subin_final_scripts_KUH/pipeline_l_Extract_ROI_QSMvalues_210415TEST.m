...
for idx = 1:length(subjects)
    ...
    subj = subjects{idx};
        fname_fsroi  = [dirs.v_z_fsbrainmask '\flip_v_crop_magcoreg_nn_' subj suffix.FSroimask '.nii'];
        fname_qsm    = [dirs.qsmnet '\' subj suffix.qsmnetout '.nii.gz'];
        gunzip(fname_qsm)
        
    fsroi = load_nii(fname_fsroi);
    qsm = load_nii([dirs.qsmnet '\' subj suffix.qsmnetout '.nii']); %반드시 load_nii로 불러야됨! untouch하면orientation 바뀌어서 mask랑 안 맞음.
    ...
    for ii = 1:length(regionnames)
        ...
        roimask = fsroi.img;
            ...
            roimask(find(ismember(roimask,ROI_label))) = 1;
                ...
                average_roi_qsmvalues_filtered(idx,ii) = mean(roi_qsmvalues_filtered_wo_zero);%             end  
    end
end
